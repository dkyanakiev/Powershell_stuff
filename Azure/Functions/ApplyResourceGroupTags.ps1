function ApplyResourceGroupTags(){
 [CmdletBinding()]
    Param(
        $ResID,
        $switch,
    ...
    $Logfile = "C:\tmp\Tags\$($ResID.ResourceGroupName).log"
    ...
    ### We have the resource we are going to check the tags###
            ...
            "Tag exists - $ResID.ResourceGroupName ; $varName.Name  "  | Out-File -FilePath $Logfile -Append -Force
            }
        else{
            ...
            if($varName.Value -ne $null -and $varName.Value -ne "" -and $varName.Value -ne " "){
                ...
                "$ResID.ResourceGroupName  ; $varName.Name  Value is empty"  | Out-File -FilePath $Logfile -Append -Force
            }
        }
      ...
      foreach($varName in $varNames){
       if($varName.Name -ne 'ResID' -and $varName.Name -ne 'switch'){
        ...
        if($switch -eq $true){    
            "Set-AzureRmResource -Tag $($array_tag.Keys) | $($array_tag.Values) -ResourceId $($ResID.ResourceId) -force "| Out-File -FilePath $Logfile -Append -Force                                      
        }else{
